...
Because all of these are fixed, the tracked release `.wasm` artifacts — and
therefore their SHA-256 hashes — are byte-identical across machines. The
expected hashes are committed in
[`contracts/expected-hashes.json`](../contracts/expected-hashes.json).
...
artifacts, and compares the tracked ones against
`contracts/expected-hashes.json`. It exits non-zero if any tracked hash drifts.
If extra `.wasm` files are produced, the script reports them so you can decide
whether to add them to the committed manifest.
...
When a tracked contract change intentionally alters the WASM, regenerate the
committed hashes and commit the result:
...
If you intentionally want to add newly built artifacts to the manifest, include
the explicit opt-in flag:

```bash
scripts/reproducible-build.sh --update --all-artifacts
git add contracts/expected-hashes.json
```
